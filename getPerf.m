...
function  [AUROCset, AUPRCset , Rec5set  , Prec5set,n_method, methodnames] = getPerf(testsetlabel,TableScores )
    ...
    Rec5set     = 0.0.*ones( 1, n_method    ); 
    Prec5set    = 0.0.*ones( 1, n_method    ); 
    ...
    for ii=1:n_method  
        ...
        topk=100; 
        CurveRecall_topk = Y./N_pos; 
        AURecallset(ii)  = sum(CurveRecall_topk(top_k_vec_t<=topk) ) ;  
        Rec5set(ii)      = CurveRecall_topk(top_k_vec_t==5)  ;   
        %
        CurvePrec_topk = Y./top_k_vec;
        AUPrecset(ii)  = sum(CurvePrec_topk(top_k_vec_t<=topk) ) ;  
        Prec5set(ii)   =  CurvePrec_topk(top_k_vec_t==5) ;   
